...
# StayEastSearchAgent 클래스 #
class StayEastSearchAgent(SearchAgent):
    def __init__(self):
        # Uniform Cost Search 알고리즘을 searchFunction으로 설정
        self.searchFunction = search.uniformCostSearch  
        
        # 위치 기반 비용 함수(costFn)를 정의 (x 좌표에 따라 비용이 달라짐)
        # pos[0]은 위치의 x 좌표를 의미, x 좌표가 커질수록 비용이 줄어드는 구조 (0.5^x)
        costFn = lambda pos: .5 ** pos[0]
        
        # searchType은 PositionSearchProblem을 사용하고, (1, 1)을 목표로 설정
        # 마지막 인자 None은 막힘 상태가 없음을 의미하고, False는 장애물이 없음을 의미
        self.searchType = lambda state: PositionSearchProblem(state, costFn, (1, 1), None, False)

# StayWestSearchAgent 클래스 #
class StayWestSearchAgent(SearchAgent):
    def __init__(self):
        ...
        # 위치 기반 비용 함수(costFn)를 정의 (x 좌표에 따라 비용이 달라짐)
        # pos[0]은 위치의 x 좌표를 의미, x 좌표가 커질수록 비용이 증가하는 구조 (2^x)
        costFn = lambda pos: 2 ** pos[0]
        
        # searchType은 PositionSearchProblem을 사용하고, 목표 위치는 기본값으로 설정
